...
def sendmail(args):
    ...
    if "To" not in message and not recipients:
        raise ValueError(
            "Message doesn't have a To header and no recipients "
...
def parse_args(argv=None):
    ...
    parser.add_argument("-bp", "-q", action="store_true", help="List the mail queue")
    parser.add_argument(
        "-bi", "-I", action="store_true", help="Initialize the alias database. Ignored."
    )
    parser.add_argument(
        "-i",
        "-oi",
    ...
    parser.add_argument(
        "-F",
        help="Set the full name of the sender. Only "
    ...
    if unparsed:
        _logger.debug("laim: ignoring the following arguments: %s", " ".join(unparsed))
